---
date: "2019-04-02T01:07:30-05:00"
title: "Installing a Highly Available OpenShift Cluster"
authors: ["redhat-cop"]
categories:
  - 'Field Guide'
tags:
...
Red Hat does major releases of OpenShift about once a quarter. As of this writing, the current major version is 3.11. Additionally, "dot releases" of OpenShift tend to be released every 4 to 6 weeks. The latest 3.11, for example, is 3.11.16-1. It's generally a good practice to decide on the major version of OpenShift to target and allow the install playbook to take the latest minor release of that major version. This will make sure you get all of the latest bug fixes and patches for that version.
...
This value will determine the maximum number of Pod IPs available to the cluster.  The Default value of /14 will provide 262,142 pod IPs for the cluster to assing to Pods. If this installation needs to use an alternative value, capture and insert into the hosts file (link:#_ansible_inventory_update[example] below).


==== Service Network CIDR
*Ansible Host File Variable:* `openshift_portal_net=172.30.0.0/16`

Each service in the cluster will be assigned an IP from this range.  The default value of /16 will provide up to 65,534 IP addresses for services. If this installation needs to use an alternative value, capture and insert into the hosts file (link:#_ansible_inventory_update[example] below).
...
*Ansible Host File Variable:* `openshift_master_api_port=443`
...
*Ansible Host File Variable:* `openshift_master_console_port=443`
...
This will be the hostname that internal components within the cluster will use to talk back to the master.
...
The Ansible config playbook will by default generate a bunch of certificates that will be used by various components. If you have need to customize these certificates, consult link:/articles/openshift-custom-certificates[Configuring OpenShift to use Custom Certificates].
...
See our link:/articles/openshift-load-balancing[Integrating External Loadbalancers] guide for more details on this.
...
openshift_master_identity_providers=[{'name': 'htpasswd_auth', 'login': 'true', 'challenge': 'true', 'kind': 'HTPasswdPasswordIdentityProvider'}] <1>
...
- Block Storage: Volumes or disks that can be mounted to only one container at a time (known as `ReadWriteOnce` mode). Examples of Block Storage are link:{docs_url}install_config/persistent_storage/persistent_storage_cinder.html[OpenStack Cinder], link:{docs_url}install_config/persistent_storage/persistent_storage_ceph_rbd.html[Ceph RBD], link:{docs_url}install_config/persistent_storage/persistent_storage_aws.html[Amazon Elastic Block Storage], link:{docs_url}install_config/persistent_storage/persistent_storage_iscsi.html[iSCSI], link:{docs_url}install_config/persistent_storage/persistent_storage_fibre_channel.html[Fibre Channel]. Most database technologies perfer block storage.
...
By default nfs will require an override within your ansible hosts file if you choose to use it for the registry:  `openshift_enable_unsupported_configurations=true`
...
* Syncing subscription-manager channels via Satellite 6 (Recommended. link:#_satellite_6[See below])
* Creating and syncing custom channels via Satellite 5 (Coming soon)
* Creating and syncing a custom RPM server (link:#_custom_yum_repos[See below])
...
* A Simple Bootstrap Registry (link:#_simple_docker_registry_docker_distribution[See below.])
* Using the OpenShift Standalone Registry (Coming Soon)
* Syncing image content to Satellite 6 (link:#_syncing_images_using_satellite_6[See below])
...
* 3 _Masters_ (openshift-master-[1-3].c1-ocp.myorg.com)
  ...
  ** 10 GB - `/var/lib/etcd` (etcd write path. Should be back by a fast disk, SSD or logical volume. In this guide, available as `/dev/vdb`.)
  ** 50 GB+ - `/var/lib/docker` (If using Docker as the container runtime, this is the container image cache formatted, configured, and managed by `docker-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `STORAGE_DRIVER=overlay2` be set in `/etc/sysconfig/docker-storage-setup`.)
  ** 50 GB+ - `/var/lib/containers` (If using CRI-O as the container runtime, this is the container image cache formatted, configured, and managed by `container-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `storage_driver=overlay` be set in `/etc/crio/crio.conf`.)
...
* 3 _Application Nodes_ (openshift-node-[1-3].c1-ocp.myorg.com)
  ...
  ** 10 GB - `/usr` (system binaries)
  ** 10 GB - `/var` (system drivers and other bits)
  ** 100 GB+ - `/var/lib/docker` (If using Docker as the container runtime, this is the container image cache formatted, configured, and managed by `docker-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `STORAGE_DRIVER=overlay2` be set in `/etc/sysconfig/docker-storage-setup`.)
  ** 100 GB+ - `/var/lib/containers` (If using CRI-O as the container runtime, this is the container image cache formatted, configured, and managed by `container-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `storage_driver=overlay` be set in `/etc/crio/crio.conf`.)
  ** 50 GB+ - `/var/lib/origin` (Ephemeral/temp storage for running containers. Should be backed by a logical volume.)
  ** 20 GB+ - `/var/log` (system logging write path)
  ** `[SWAP]` (should be disabled and not partitioned for)
...
Each of these servers should be provisioned with an SSH public key which can be used to access all hosts from the Ansible Control Host. Further setup of keys and their propagation is covered below.
...
subscription-manager repos --disable="*" --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-3.11-rpms" --enable="rhel-7-fast-datapath-rpms"  --enable="rhel-7-server-ansible-2.6-rpms"
----
...
* Provisioning infrastructure on Amazon EC2 using the awscli (Coming Soon)

==== Configuring Node Host Labels

Labels are simple key/value pairs that are used to organize, group, or select API objects. You can assign labels to node hosts during the Ansible install by configuring the /etc/ansible/hosts file. Labels are useful for determining the placement of pods onto nodes using the scheduler. Other than `openshift_node_group_name="node-config-compute` (discussed in link:#_configuring_labels_for_dedicated_infrastructure_nodes[Configuring Labels For Dedicated Infrastructure Nodes]]), the actual label names and values are arbitrary and can be assigned however you see fit per your cluster’s requirements.

To assign labels to a node host during the advanced installation, use the `openshift_node_group_name` variable with the desired labels added to the desired node host entry in the `[nodes]` section. For example:

----
[nodes]
openshift-appnode-1.c1-ocp.myorg.com openshift_node_group_name="node-config-compute"
openshift-appnode-2.c1-ocp.myorg.com openshift_node_group_name="node-config-compute"
----

===== Configuring Labels For Application Nodes

The `osm_default_node_selector` Ansible setting determines the label that projects will use by default when placing pods on Application nodes. It is set to `node-role.kubernetes.io/compute=true` by default:
...
The `openshift_hosted_router_selector` and `openshift_hosted_registry_selector` Ansible settings determine the label selectors used when placing registry and router pods. They are set to `node-role.kubernetes.io/infra=true` by default:
...
[nodes]
openshift-master-[1:3].c1-ocp.myorg.com openshift_node_group_name="node-config-master"
openshift-infranode-[1:3].c1-ocp.myorg.com openshift_node_group_name="node-config-infra"
openshift-appnode-1.c1-ocp.myorg.com openshift_node_group_name="node-config-compute"
openshift-appnode-2.c1-ocp.myorg.com openshift_node_group_name="node-config-compute"
----
...
./docker-registry-sync.py --from=registry.access.redhat.com --to=registry.c1-ocp.myorg.com:5000 --file=./docker_tags.json --openshift-version=3.11
----
...
hammer repository-set enable --organization "c1-ocp.myorg.com" --product "Red Hat Enterprise Linux Server" --name "Red Hat OpenShift Container Platform 3.11(RPMs)" --releasever "" --basearch "x86_64"
...
hammer repository-set enable --organization "c1-ocp.myorg.com" --product "Red Hat Enterprise Linux Server" --name "Red Hat Ansible Engine 2.6 RPMs for Red Hat Enterprise Linux 7 Server" --releasever "7Server" --basearch "x86_64"
...
hammer repository synchronize --name "Red Hat OpenShift Container Platform 3.11 RPMs x86_64" --organization "c1-ocp.myorg.com"
...
hammer repository synchronize --name "Red Hat Ansible Engine 2.6 RPMs for Red Hat Enterprise Linux 7 Server" --organization "c1-ocp.myorg.com"
----
...
The procedure for creating custom yum repos is documented in the link:{docs_url}install/disconnected_install.html#disconnected-syncing-repos[Official Documentation]

==== Subscribing Directly to Red Hat

The process for subscribing directly to Red Hat is covered in the link:{docs_url}install/host_preparation.html#host-registration[Official Documentation].
...
ansible -i c1-ocp.myorg.com/hosts OSEv3 -a 'subscription-manager repos --disable="*" --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-3.11-rpms" --enable="rhel-7-fast-datapath-rpms" --enable="rhel-7-server-ansible-2.6-rpms"'
----
...
During the link:#_provision_servers[Provision Servers] step of this guide, we provisioned all of our nodes (including the masters) with docker volumes attached as `/dev/vdb`. We'll now install and configure docker to use that volume for all local docker storage.

NOTE: There are other options for configuring docker storage. They are outlined in the link:{docs_url}install/host_preparation.html#configuring-docker-storage[Official Docs].
...
Just as with the Docker Storage, during the link:#_provision_servers[Provision Servers] step of this guide, we provisioned our masters and nodes with an extra volume to be used for `/var/lib/etcd` (for masters) and `/var/lib/origin` (for nodes), attached as `/dev/vdc` in this guide. (Make sure to replace this with the disk available in your environment.) We will now demonstrate the steps involved with using LVM to set up and use this volume for backing storage.
...
OpenShift has tunable parameters to further enhance and protect the cluster/nodes.  These parameters should be added to your ansible hosts file.
...
*  kubeletArguments.kube-reserved = Resources reserved for node components
*  kubeletArguments.system-reserved = Resources reserved for the remaining system components
*  kubeletArguments.maximum-dead-containers-per-container = The number of instances to retain per pod container
*  kubeletArguments.maximum-dead-containers = node removes the dead containers, all files inside those containers are removed as well
*  kubeletArguments.pods-per-core = How many pods will be allowed to run per core
*  kubeletArguments.max-pods = Max number of pods on a node
*  kubeletArguments.image-gc-high-threshold = The percent of disk usage which triggers image garbage collection
*  kubeletArguments.image-gc-low-threshold = The percent of disk usage to which image garbage collection attempts to free

=== Validating Pre-requisites

Once we have everything prepped, it's a good idea to run through our link:/articles/openshift-pre-install[OpenShift Pre-Install Validation Checklist].
...
link:/articles/openshift-validate[Validating an OpenShift Install]

== What's Next?

* link:/articles/openshift-admin-overview[Overview of OpenShift for Administrators]
* link:/articles/openshift-custom-certificates[Configuring OpenShift to use Custom Certificates]
